...
###### 結合に関する細かい話
Scala風の擬似コードを使って書けば、二項関係のデータ構造は、Set[(A,B)]
といったものになります。
Set[(A,B)]から、(A => Set[B])というインターフェースは素直に取り出すことができます。また、 入力となる型Aの値の集合が与えられていれば、(A => Set[B]) からSet[(A,B)]を再び取り出すことも可能です。つまり相互に変換できるため、この違いは無視してしまうことができます。これを踏まえて二項関係の結合は、次のような擬似コードによる関数で表されます。

def >>>[A,B,C](f:A => Set[B], g:B => Set[C]) = (x:A) => f(x).flatMap(g)
